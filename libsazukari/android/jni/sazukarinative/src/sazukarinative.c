...
slength_t socket_rd(byte * buf, length_t sz, void *param)
{
  ...
  if (!jbuf) {
    __android_log_print(ANDROID_LOG_WARN, "SAZUKARI",
                        "Could not allocate ByteBuffer");
    return (-1);
  }

  return (*g_env)->CallStaticLongMethod(g_env, g_myclass,
                                        g_methods[sn_SocketRead], p->socket,
                                        jbuf, (jlong) sz);
}
...
slength_t socket_wr(byte * buf, length_t sz, void *param)
{
  ...
  return (*g_env)->CallStaticLongMethod(g_env, g_myclass,
                                        g_methods[sn_SocketWrite], p->socket,
                                        jbuf, (jlong) sz);
}

jint JNI_OnLoad(JavaVM * vm, void *nothing)
{
  __android_log_print(ANDROID_LOG_DEBUG, "SAZUKARI",
                      "Initializing libsazukarinative");
  ...
  for (i = 0; i < SN_METHODCOUNT; ++i) {
    jmethodID id =
        (*env)->GetStaticMethodID(env, parent, g_methoddesc[i].name,
                                  g_methoddesc[i].sig);
    if (!id) {
      __android_log_print(ANDROID_LOG_WARN, "SAZUKARI",
                          "Method %s[%s] not found", g_methoddesc[i].name,
                          g_methoddesc[i].sig);
      goto destroy_ref;
    }
    g_methods[i] = id;
...
jlong Java_com_nmi_sazukari_SazukariNative__1szkrGetSessionDataSize(JNIEnv *
                                                                    env,
                                                                    jclass
                                                                    clazz)
{
...
jint Java_com_nmi_sazukari_SazukariNative__1szkrGetSessionData(JNIEnv * env,
                                                               jclass clazz,
                                                               jint ctxHandle,
                                                               jobject
                                                               outSessionData)
...
jint Java_com_nmi_sazukari_SazukariNative__1szkrResetContext(JNIEnv * env,
                                                             jclass clazz,
                                                             int ctxHandle)
{
  szkr_ctx_t *ctx = g_contexts[ctxHandle].sazukari;
...
jint Java_com_nmi_sazukari_SazukariNative__1szkrHandshake(JNIEnv * env,
                                                          jclass clazz,
                                                          int ctxHandle)
{
  szkr_ctx_t *ctx = g_contexts[ctxHandle].sazukari;
...
jint Java_com_nmi_sazukari_SazukariNative__1szkrLastError(JNIEnv * env,
                                                          jclass clazz,
                                                          int ctxHandle)
{
  szkr_ctx_t *ctx = g_contexts[ctxHandle].sazukari;
...
jint Java_com_nmi_sazukari_SazukariNative__1szkrResume(JNIEnv * env,
                                                       jclass clazz,
                                                       int ctxHandle,
                                                       jobject sessionData)
{
...
jlong Java_com_nmi_sazukari_SazukariNative__1szkrSendMessage(JNIEnv * env,
                                                             jclass clazz,
                                                             int ctxHandle,
                                                             jobject msg)
{
...
jobject Java_com_nmi_sazukari_SazukariNative__1szkrAccessResponseBuffer(JNIEnv *
                                                                        env,
                                                                        jclass
                                                                        clazz,
                                                                        int
...
void Java_com_nmi_sazukari_SazukariNative__1szkrDestroyResponseBuffer(JNIEnv *
                                                                      env,
                                                                      jclass
                                                                      clazz,
                                                                      int
...
void Java_com_nmi_sazukari_SazukariNative__1szkrDestroyCtx(JNIEnv * env,
                                                           jclass clazz,
                                                           int ctxHandle)
{
...
void Java_com_nmi_sazukari_SazukariNative__1szkrDestroy(JNIEnv * env,
                                                        jclass clazz)
{
  int i;
